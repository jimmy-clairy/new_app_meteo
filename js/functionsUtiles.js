...
/**
 * Splits the date string from a weather data item and returns an array of numeric date components.
 * @param {Object} listItem - The weather data item.
 * @returns {Array} - Array containing numeric date components [year, month, day].
 */
export function numericDate(listItem) {
    return listItem.dt_txt.split(' ')[0].split('-');
}